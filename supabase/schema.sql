...
-- Auto-create profile on signup (assigns admin role to the admin UID)
...
-- Book covers: authenticated users can upload
...
-- Book covers: users can update own files
...
CREATE POLICY "Users can update own book covers"
  ...
  USING (bucket_id = 'book_covers' AND auth.uid()::text = (storage.foldername(name))[1]);

-- Book covers: users can delete own files
